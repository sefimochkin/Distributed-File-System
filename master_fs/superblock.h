...
extern void lock_fs_mutex(FS_Handler * fs_handler);
extern void unlock_fs_mutex(FS_Handler * fs_handler);


struct inode * get_free_inode(struct superblock *sb, FS_Handler * fs_handler);
struct block* get_free_block(struct superblock *sb, FS_Handler * fs_handler);

struct block* get_n_continuous_free_blocks(struct superblock *sb, int number_of_blocks, FS_Handler * fs_handler);

void free_inode(struct superblock *sb, struct inode* inode, FS_Handler *fs_handler, int id);
void free_block(struct superblock *sb, struct block* block, FS_Handler *fs_handler);
