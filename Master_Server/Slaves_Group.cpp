...
void Slaves_Group::parse_command_and_do_something(std::string message){
    ...
    if (command.find(std::string("send")) == 0)
        ...
        files[first_arg].index_of_file = std::stoi(index_of_file);
        files[first_arg].size_of_file_in_blocks = std::stoi(size_in_blocks);
        files[first_arg].rw_mtx->unlock();

        counter_mutex->lock();
        all_free_blocks -= files[first_arg].size_of_file_in_blocks;
        counter_mutex->unlock();

        for (auto &it: slaves_info) {
            if (it.second.id == files[first_arg].storage_slave_id) {
        ...
        for (auto &it: slaves_info) {
            if (it.second.id == files[first_arg].storage_slave_id) {
                it.second.counter_mutex->lock();
                it.second.free_blocks += files[first_arg].size_of_file_in_blocks;
                it.second.counter_mutex->unlock();
                break;
            }
        }
