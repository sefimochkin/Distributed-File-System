...
int get_number_of_address_blocks(struct superblock *sb, int number_of_adresses){
    ...
    if (number_of_adresses < sb->number_of_bytes_in_block)
        added_block = 1;
    if ((number_of_adresses / sb->number_of_bytes_in_block) % (sb->number_of_bytes_in_block / sb->number_of_chars_in_index) > 0)
        added_block = 1;
...
void free_data_in_blocks(struct superblock *sb, struct block* address_blocks, int size_of_data){
